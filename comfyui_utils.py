...
def download_and_save_images(bundle) -> None:
    """
    ...
    for i in range(2):
        ...
        image_path = bundle[i][1]
        image_url = bundle[i][0]

        if image_path and image_url:
            print(f"DOWNLOADING REFERENCE IMAGE {i+1} FROM {image_url}")

            response = requests.get(image_url)

            if response.status_code == 200:
                print(f"GOT THE REFERENCE IMAGE {i+1} FROM UPLOADCARE")
                with open(image_path, "wb") as f:
                    f.write(response.content)
                print(f"SAVED IMAGE {i+1} TO {image_path}")
            else:
                print(f"FAILED TO GET UPLOADCARE IMAGE {i+1}: {image_url}")
        else:
            print(f"REFERENCE IMAGE {i+1} NOT FOUND")
